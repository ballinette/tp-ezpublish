...
class eZCountryType extends eZDataType
{
    ...
    */
    ...
    {
        $multiple = $attributeParametersNode->getElementsByTagName( 'multiple-choice' )->item( 0 )->textContent;
        $defaultCountries = $attributeParametersNode->getElementsByTagName( 'default-countries' )->item( 0 )->textContent;
        $classAttribute->setAttribute( self::MULTIPLE_CHOICE_FIELD, (int)$multiple );
        $classAttribute->setAttribute( self::DEFAULT_LIST_FIELD, $defaultCountries);
    }

    function serializeContentClassAttribute( $classAttribute, $attributeNode, $attributeParametersNode )
    {
        $multiple = $classAttribute->attribute( self::MULTIPLE_CHOICE_FIELD );
        $defaultCountries = $classAttribute->attribute( self::DEFAULT_LIST_FIELD );
        $dom = $attributeParametersNode->ownerDocument;
        $multipleNode = $dom->createElement( 'multiple-choice' );
        $multipleNode->appendChild( $dom->createTextNode( $multiple ) );
        $attributeParametersNode->appendChild( $multipleNode );
        $defaultCountriesNode = $dom->createElement( 'default-countries' );
        if ( $defaultCountries )
        {
            $defaultCountriesNode->appendChild( $dom->createTextNode( $defaultCountries ) );
        }
        $attributeParametersNode->appendChild( $defaultCountriesNode );
    }
